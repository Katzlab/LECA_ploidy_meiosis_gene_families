...
def correct_tree(fname): 
	...
	tree_corrected = open (fname.split('.treefile')[0] + '_correcting.tre', 'w')
...
def read_newick(input_handle, use_minor_clades, bacterial_bin, directory, rooting):
	...
	temp_handle = input_handle.replace('.treefile', '_temp.tre')
	...
	if(rooting == 0):
		...
		tree_file = temp_handle.split('.treefile')[0] + '_correcting.tre'
		read(tree_file) 
		tree = var.trees[0]
