...
def boxplot_with_seaborn(some_tsv): 
    ...
    ax = sns.violinplot(data=slen_df, x='OG', y='Length', hue='Domain', showfliers=False)
    plt.show()
...
def deal_w_input(path, og_number, s_size = 0, runs = 0):
    ...
    if (len(og_fasta_file) != len(og_number)):
        print("\nYou need an og number for each fasta file you inputted, please fix the og_number list or the fasta inputs and then try again")
        print("(1) Your og_number list currently contains: " + str(og_number))
        print("(2) Your fasta inputs list currently includes: [", end='')
        for item in og_fasta_file:
            print(str(item.split("/")[-1]), end='')
            if item == og_fasta_file[-1]:
                print(" ]\n")
            else:
                print(", ", end ='')
        exit()
    ...
    #else:
        #key = {next((s for s in og_fasta_file if v in s), None): i for i, v in enumerate(og_number)}
        #sorted(og_fasta_file, key=key.get) #???
        #print(og_fasta_file)
...
if __name__ == '__main__':
    ...
    path = "/Users/emmaschumacher/Desktop/Testnewscript/OGDatamulti/"
