...
def get_sampsize(og_fasta_file):
	...
	print(og_fasta_file[(og_fasta_file.index('cdHit/') + 5):(og_fasta_file.index('_trans'))] + " " + str(s_size) + " " + str(i)) 
...
def get_random(og_fasta_file, ssize, multi = None):
	...
	if not multi:
		name = 'random_subsamps/' + og_fasta_file[og_fasta_file.index('cdHit/') + 5:og_fasta_file.index(".fasta")] + "smallerseq.fasta"
	else:
		name = 'random_subsamps/' + og_fasta_file[og_fasta_file.index('cdHit/') + 5:og_fasta_file.index(".fasta")] + str(multi) + "smallerseq.fasta" 
